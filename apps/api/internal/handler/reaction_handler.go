...
func (h *ReactionHandler) GetCommentReactionSummary(c *gin.Context) {
	...
	response := dto.ReactionSummaryResponse{
		Counts: mapper.ToCommentReactionCounts(counts),
	}
...
func (h *ReactionHandler) ReactToReview(c *gin.Context) {
	...
	reaction, err := h.reactionSvc.ReactToReview(c.Request.Context(), userID, int32(reviewID), db.ReactionType(req.Type))
	if err != nil {
		log.Printf("react to review error: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "internal server error"})
		return
	}
...
func (h *ReactionHandler) RemoveReviewReaction(c *gin.Context) {
	reviewID, err := strconv.Atoi(c.Param("reviewId"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid review id"})
		return
	...
	if err := h.reactionSvc.RemoveReviewReaction(c.Request.Context(), userID, int32(reviewID)); err != nil {
		if errors.Is(err, service.ErrReactionNotFound) {
			c.JSON(http.StatusNotFound, gin.H{"error": "reaction not found"})
			return
		}
		log.Printf("remove review reaction error: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "internal server error"})
		return
	}
...
func (h *ReactionHandler) GetReviewReactions(c *gin.Context) {
	reviewID, err := strconv.Atoi(c.Param("reviewId"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid review id"})
		return
	...
	reactions, err := h.reactionSvc.GetReviewReactions(c.Request.Context(), int32(reviewID), int32(limit), int32(offset))
	if err != nil {
		log.Printf("get review reactions error: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "internal server error"})
		return
	}

	c.JSON(http.StatusOK, gin.H{"data": mapper.ToReviewReactionResponses(reactions)})
}

func (h *ReactionHandler) GetReviewReactionSummary(c *gin.Context) {
	reviewID, err := strconv.Atoi(c.Param("reviewId"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid review id"})
		return
	...
	counts, err := h.reactionSvc.GetReviewReactionCounts(c.Request.Context(), int32(reviewID))
	if err != nil {
		log.Printf("get review reaction counts error: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "internal server error"})
		return
	}

	response := dto.ReactionSummaryResponse{
		Counts: mapper.ToReviewReactionCounts(counts),
	}

	// If authenticated, include user's reaction
	if userID, exists := c.Get("user_id"); exists {
		userReaction, err := h.reactionSvc.GetUserReviewReaction(c.Request.Context(), userID.(int32), int32(reviewID))
		if err != nil {
			log.Printf("get user review reaction error: %v", err)
		} else if userReaction != nil {
			reactionType := string(userReaction.Type)
			response.UserReaction = &reactionType
		}
